...
if __name__ == '__main__':
    ...
    session.commit()
    print("Count after deletion:", session.query(Game).count())
    ...
    session.commit()
    print("Count after adding random games:", session.query(Game).count())

    session.bulk_save_objects([botw, ffvii, mk8, ccs])
    session.commit()
    print("Count after adding specific games:", session.query(Game).count())
